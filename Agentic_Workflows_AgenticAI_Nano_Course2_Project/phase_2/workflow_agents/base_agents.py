...
class DirectPromptAgent:
    ...
    def respond(self, prompt):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "user", "content": prompt} # Provide the user's prompt here. Do not add a system prompt.
            ],
            temperature=0
        )
    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": """Forget any previous conversational context. 
                You need to assume the following defined persona: {}""".format(self.persona)},
...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions): #Set loop to iterate up to the maximum number of interactions:
            ...
            print(f"Prompt:\n{prompt_to_evaluate}")
            response_from_worker = self.worker_agent.respond(prompt_to_evaluate) # Obtain a response from the worker agent
            print(f"Worker Agent Response:\n{response_from_worker}")

            print(" Step 2: Evaluator agent judges the response")
            eval_prompt = (
                f"Does the following answer: {response_from_worker}\n"
                f"Meet this criteria: {self.evaluation_criteria}\n"  # Insert evaluation criteria here
                f"Respond Yes or No, and the reason why it does or doesn't meet the criteria."
            )
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "system", "content": f"You are an evaluator agent with the persona: {self.persona}"},
                          {"role": "user", "content": eval_prompt}],  #  Define the message structure sent to the LLM for evaluation (use temperature=0)
                temperature = 0
            )
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages= [{"role":"system","content":"You are a correction assistant that provides concise, "
                                                          "helpful and actionable instructions on how to fix answers."},
                               {"role":"user", "content": instruction_prompt}],
                    temperature = 0#  Define the message structure sent to the LLM to generate correction instructions (use temperature=0)
                )
...
class RoutingAgent():
    ...
    def __init__(self, openai_api_key, agents):
        """
            Initializes the Routes prompts to the best agent.

            Params:
                openai_api_key (str): secret key for openai
                agents (list): list of dictionaries that include fields: name (str), description (str), func (callable)

        """
        # Initialize the agent with given attributes
        self.openai_api_key = openai_api_key
        self.agents = agents # Define an attribute to hold the agents, call it agents
    ...
    def route_prompts(self, user_prompt):# Define a method to route user prompts to the appropriate agent
        ...
        for agent in self.agents:
            agent_emb =self.get_embedding(agent["description"])# Compute the embedding of the agent description
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        "content":(f"You are an action planning agent. Using your knowledge, you extract from the user prompt the steps "
         f"requested to complete the action the user is asking for. You return the steps as a list. Only return "
         f"the steps in your knowledge. Forget any previous context. This is your knowledge: {self.knowledge}")
            },
                      {"role":"user","content":prompt}],
        temperature=0)


        response_text = response.choices[0].message.content.strip()  # Extract the response text from the OpenAI API response

        # Clean and format the extracted steps by removing empty lines and unwanted text
        steps = []
        for i in response_text.split("\n"):
            clean_text = re.sub(pattern=r'[^A-Za-z0-9 ]+',
                       repl="",
                       string=i.strip()).strip()
            if clean_text != "":
