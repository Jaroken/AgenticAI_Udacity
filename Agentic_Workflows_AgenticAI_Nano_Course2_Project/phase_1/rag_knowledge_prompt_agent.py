# Import RAGKnowledgePromptAgent
import os
...
RAG_knowledge_prompt_agent = RAGKnowledgePromptAgent(openai_api_key=openai_api_key, persona=persona)# Instantiate RAGKnowledgePromptAgent
...
chunks = RAG_knowledge_prompt_agent.chunk_text(text=knowledge_text) # Compute the knowledge_text the chunks
embbedings = RAG_knowledge_prompt_agent.calculate_embeddings() # Compute the knowledge_text the chunk embeddings

prompt = "What is the podcast that Clara hosts about?"
# Print the prompt and the response
print(f"Prompt: {prompt}")
print(f"Rag Agent Response: {RAG_knowledge_prompt_agent.find_prompt_in_knowledge(prompt=prompt)}")